...
const IMAGE_BYTES: usize = IMAGE_ROW_BYTES * (IMAGE_DIMENSION as usize);
...
pub fn load_spritesheet<'a>(
    ...
    return pic_data
        ...
        // Ignore EGA_HEADER (4 bytes) and garbage between images
        .map(|x| &x[EGA_HEADER.len()..IMAGE_BYTES])
                ...
                .for_each(|(i, x)| {
                    ...
                    surface
                        .fill_rect(
                            ...
                            // (IMAGE_CHANNEL_ROW_CRUMBS) instead of the actual row size (IMAGE_DIMENSION)
                            pixel(
                                (i % IMAGE_CHANNEL_ROW_CRUMBS) as i32,
                                (i / IMAGE_ROW_CRUMBS) as i32,
                            ),
                            EGA_PALETTE[x as usize],
                        )
                        .unwrap()
                });
